...
Reads `data/raw/wikisource/vol_01/vol01-page{0010..0023}.json` and emits a
fully static `tools/viewer/preface.html` — same shell as the ancillary
pages, with the body, TOC, and footnotes pre-rendered.

The wikitext → HTML step is `ancillary_render.render_pages` — the SAME
preprocess → walker → renderer the corpus goes through.  This file owns
only what is page-specific: which leaves, the per-page corrections, the
masthead-title drop (the page header already says "Editorial Preface"),
and the shell/TOC/Notes assembly.  (It used to own a parallel regex
mini-pipeline — noinclude strip, template unwrappers, a catch-all
`{{…}}` strip, its own paired-regex marker decoder — sweeper-campaign
item K1; all of it is dissolved into the one pipeline, which also
recognizes what the regexes never did: the author links, the article
links, and the outline structures now render instead of flattening.)
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .preface-body p:first-of-type {{ text-indent: 0; }}
    /* The print's drop initial ({{{{dropinitial|I}}}}): the pipeline carries the
       LETTER as content and treats the sizing as droppable print furniture, so
       the drop-cap look is page presentation — ::first-letter, no markup. */
...
def build_toc_html(toc: list[tuple[str, str]]) -> str:
    if not toc:
        return ""
    items = "".join(
        f'<li><a href="#{sid}">{label}.</a></li>' for sid, label in toc)
...
def main() -> int:
    ...
    page = PAGE_TEMPLATE.format(
        ...
        BODY=doc.body_html,
        FOOTNOTES=footnotes_html(doc.footnotes),
    )
    OUTPUT.write_text(page, encoding="utf-8")
    print(f"Wrote {OUTPUT}: {len(page):,} chars; "
          f"{len(doc.toc)} shoulder headings; {len(doc.footnotes)} footnotes")
    return 0
