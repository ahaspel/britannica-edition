...
- **✅ DONE — `_CHROME_EMPTY_NAMES` drop-list dissolved** (`_classifier.py`). Front matter is excluded at the leaf gather (manifest), so its 12 front-matter / vol-29 names were deleted as dead (0 in-range); the page furniture (page heading / pagenum / ambox / hidden text) moved to the preprocess strip (`_strip_chrome_furniture`); the printed-TOC rows went to the `_toc` producer. The "separate look" at the maintenance templates found **3 content-loss bugs the drop-list was hiding** — `suspect` (displays the flagged word) → FRAME, `main other` (namespace-switch prose) → a param-1 producer, `lps`/`lpe` (split-word) → the split-word producer. The set and its `classify` branch are gone; the downstream `_LEAD_CHROME` page-heading clause (now dead, 0/4825) was trimmed too.
- **✅ DONE — split-word reconstruction is a producer** (`_splitword.py`). The `_HWS_STANDALONE`/`_HWE_STANDALONE` sweep is deleted; `{{hws}}`/`{{hwe}}` and their named-param twins `{{lps}}`/`{{lpe}}` reconstruct by recognition in one `process_split_word` (start marker → the rejoined word from positional *or* named form, end marker → empty). `hws`/`hwe` output byte-identical.
...
- **Standing QA / regression net (no caller but prized):** `quality_report.py`, `leak_audit.py`, `snapshot_corpus.py`, `walker_snapshot.py`, `check_quality_gate.py`, `xref_coverage_audit.py`, `verify_known_articles.py`, the label-distribution/snapshot family, `tools/qa/*`, `tools/render_article.py`.  (`check_table_path_purity.py` and `layout_wrapper_contents.py` were listed here as prized until 2026-08-18, when running them showed both had been dead on IMPORT since the chem sub-classification they audited was deleted.  A document asserting a net is not evidence the net runs.  Deleted; git keeps them.)
