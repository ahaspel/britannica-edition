# Britannica Edition — Status (HISTORY)

Archived progress log moved out of `docs/status.md` on 2026-06-13.  These are the
completed-arc narratives (2026-06-09 back to 2026-05-17), the old `NEXT ARCS`, the
2026-05-17 rebuild record, and the pre-campaign `Queued` / `Known issues` lists, kept
verbatim for reference.  **The Known-issues / Queued entries predate the recursive-
architecture campaign and reference now-deleted labels — treat them as unconfirmed
history, not a live to-do list.**  Current state and durable reference live in
